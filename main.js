...
var cert = JSON.parse(
  new Buffer(process.env.FIREBASE_SERVICE_ACCOUNT, "base64").toString("ascii")
);
...
async function discordGetCategory(category_name) {
  var guild = await discordClient.guilds.fetch(
  ...
  if (category_name) {
    category = guild.channels.cache.find(
      (c) =>
        c.name.toLowerCase().trim() == category_name.toLowerCase().trim() &&
...
async function discordCreateChannel(
  ...
  await guild.channels.cache.forEach(async function (channel) {
    if (
      ...
      channel.parent.name.trim().toLowerCase() ==
        category_name.trim().toLowerCase()
    )
      new_channel = channel;
  });
  if (recreate && new_channel) {
    await new_channel.delete();
    new_channel = null;
  }
  if (!new_channel)
    new_channel = await guild.channels.create(name, {
      name: name,
      type: type,
      userLimit: 5,
  ...
  addition_user.forEach(async function (user) {
...
async function discordClearChannel(
  name = [],
  type = "GUILD_VOICE",
  category_name = null
) {
...
async function discordDeleteUser(interaction, uid) {
  try {
    ...
    msg.delete();
    var user = (
      await database.ref(`/private_users/${uid}/`).once("value")
    ).val();
    var confirm_msg = await msg.channel.send(
      `\`\`\`Are you sure? Delete account ${user.name} (${user.username})\`\`\``
    );
    await Promise.all([confirm_msg.react("✅"), confirm_msg.react("❎")]);
    const filter = (reaction, user) => {
      return (
        ["✅", "❎"].includes(reaction.emoji.name) &&
        user.id != confirm_msg.author.id
      );
    };
    await confirm_msg
...
async function discordLockAccount(interaction, uid) {
  try {
    ...
    var confirm_msg = await msg.channel.send(
      `\`\`\`Are you sure? Lock account ${user.name} (${user.username})\`\`\``
    );
    await Promise.all([confirm_msg.react("✅"), confirm_msg.react("❎")]);
    const filter = (reaction, user) => {
      return (
    ...
    var collected = await confirm_msg
      .awaitReactions({ filter, max: 1, time: 60000, errors: ["time"] })
      .catch(() => {
        throw new Error("```You do not have any react.```");
      });
    ...
    switch (reaction.emoji.name) {
      case "✅":
        ...
        collected = await days_msg.channel
        ...
        if (isNaN(days)) throw new Error("```Number of days is not valid.```");
        var reason_msg = await confirm_msg.channel.send(
          `\`\`\`You'll lock this account ${days} day(s). Why?\`\`\``
        );
        collected = await reason_msg.channel
          .awaitMessages({ filter2, max: 1, time: 60000, errors: ["time"] })
          .catch(() => {
            throw new Error("```You do not have any message.```");
          });
        ...
        var noti_msg = await confirm_msg.channel.send(
          `\`\`\`You'll lock this account ${days} day(s). Reason: ${reason}.\`\`\``
        );
        functions
          .accountLockAccount(uid, days * 24 * 60, reason)
...
async function discordUnlockAccount(interaction, uid) {
  try {
    ...
    var confirm_msg = await msg.channel.send(
      `\`\`\`Are you sure? Unlock account ${user.name} (${user.username})\`\`\``
    );
    await Promise.all([confirm_msg.react("✅"), confirm_msg.react("❎")]);
    const filter = (reaction, user) => {
      return (
    ...
    case "/acc":
      ...
      if (Object.values(data).length == 0) {
        ...
        var mess = new MessageEmbed()
          ...
          .addFields(
            ...
            },
            {
              name: "School",
              value: user.school
                ? `${user.school.schoolName} - ${user.school.provinceName}`
                : "null",
            },
            {
              name: "Creation time",
              value:
                moment(user.created_at, "X")
                  .utcOffset("+0700")
            ...
            },
            {
              ...
              value:
                moment(authUser.metadata.lastSignInTime)
                  .utcOffset("+0700")
                  .format("DD/MM/YYYY HH:mm:ss") || "null",
            }
          )
        ...
        if (user.ip) {
          ...
          for (var cur_user of Object.values(users))
            if (cur_user.name && cur_user.username != username && ++cnt <= 10)
              value += `${cur_user.name} (${cur_user.username})${
                user.school ? " - " + cur_user.school.schoolName : ""
              } - ${utils.Permission[cur_user.permission]}\n`;
          if (value) mess.addField("Account with same IP", value);
        ...
        if (!isLocked)
          ...
          row.addComponents(
            new MessageButton()
              .setCustomId(`unlock_${uid}`)
              .setStyle("PRIMARY")
              .setLabel("Mở khóa")
          );
        row.addComponents(
          new MessageButton()
            .setCustomId(`delete_${uid}`)
            .setStyle("DANGER")
            .setLabel("Xóa")
        );
      ...
      if (Object.values(data).length == 0) {
        ...
        if (username) {
          ...
          for (var cur_user of Object.values(users))
            if (cur_user.name && cur_user.username != username && ++cnt <= 10)
              value += `${cur_user.name} (${cur_user.username})${
                cur_user.school ? " - " + cur_user.school.schoolName : ""
              } - ${utils.Permission[cur_user.permission]}\n`;
          if (value) mess.addField("Account with same name", value);
        }
        if (user.ip) {
          var ip = user.ip.ip || user.ip.query;
          users = {};
        ...
        } else await msg.channel.send(`<@${msg.author.id}>`);
        const row = new MessageActionRow()
          .addComponents(
            new MessageButton()
        ...
        const reviewerId = (
          await database
            .ref(`/data/schools/${user.school.schoolId}/reviewerId`)
            .once("value")
        ).val();
        if (reviewerId) {
          const joReviewer = await database
            .ref(`/private_users/${reviewerId}/discord`)
            .once("value");
          if (joReviewer.exists()) {
            const reviewer = joReviewer.val();
            const user = await discordClient.users.fetch(reviewer.id);
            // create channel for review
  ...
  switch (content.split(" ")[0].trim().toLowerCase()) {
    ...
    case "/relink-all":
      msg.guild.members.cache.forEach(async function (member) {
        if (!member.bot) linkIOTAccount(member, false);
      });
      break;
    case "/something":
      break;
...
async function discordProcessMessage(msg) {
  ...
  // --- Check Regex ---
  var regexp_emoji = /^:[^\s:\\\/]+?:$/;
  if (regexp_emoji.test(content)) discordSendEmoji(msg);
}
...
async function discordSendEmoji(msg) {
  ...
  if (data_emoji[name]) msg.channel.send(data_emoji[name].image);
}
...
async function linkIOTAccount(member, welcome_message = true) {
  ...
  var channel = discordClient.channels.cache.find(
    (c) => c.name.toLowerCase().trim() == "general"
  );
  var uid = member.id;
  var users =
    (
  ...
  if (users.length > 0) {
    ...
    switch (user.permission) {
      case 10:
        await member.roles.add(
          member.guild.roles.cache.find((r) => r.name === "admin")
        );
        break;
      case 5:
        await member.roles.add(
          member.guild.roles.cache.find((r) => r.name === "moderator")
        );
        break;
      case 2:
        await member.roles.add(
          member.guild.roles.cache.find((r) => r.name === "verified-player")
        );
        break;
    }
    if (user.talent)
      switch (user.talent.rank) {
        case 0:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "rank-rookie")
          );
          break;
        case 1:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "rank-bronze")
          );
          break;
        case 2:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "rank-silver")
          );
          break;
        case 3:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "rank-gold")
          );
          break;
        case 4:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "rank-platinum")
          );
          break;
        case 5:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "rank-diamond")
          );
          break;
        case 6:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "rank-crown")
          );
          break;
        case 7:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "rank-ace")
          );
          break;
      }
    ...
    if (user.plan && user.plan > 0)
      switch (user.plan) {
        case 1:
          await member.roles.add(
            member.guild.roles.cache.find((r) => r.name === "elite")
          );
          break;
        case 2:
          await member.roles.add(
    ...
    if (user.reviewer) {
      await member.roles.add(
        member.guild.roles.cache.find((r) => r.name === "reviewer")
      );
    }
    if (welcome_message)
      channel.send(
        `Chào mừng người chơi IOT ${user.username} (${utils.getRankGradeName(
          user.talent
        )}) tham gia server ${member} :heart_eyes_cat:`
      );
  }
}
...
function generateIOTProfile(uid) {
  return new Promise(async function (resolve, reject) {
    ...
    if (public_user.permission < 1 || public_user.block_time) banned = "banned";
    if (public_user.name.length > 15) {
      var names = public_user.name.trim().split(" ");
      public_user.name = `${names[names.length - 2]} ${
        names[names.length - 1]
      }`;
    }
    params["user"] = {
      avatar:
        public_user.avatarUrl ||
        "https://iot.chinhphucvn.com/img/user-avatar.png",
...
discordClient.on("messageCreate", async function (msg) {
  ...
  if (
    !msg.author.bot &&
    (msg.content.toLowerCase().indexOf("baymax") > -1 ||
      msg.content.indexOf(`<@!${discordClient.user.id}>`) > -1)
  ) {
    var msg_text =
      bot_config.GREETING_MSG[
        Math.floor(Math.random() * bot_config.GREETING_MSG.length)
      ];
    var lang =
      bot_config.GREETING_LANGUAGES[
        Math.floor(Math.random() * bot_config.GREETING_LANGUAGES.length)
      ];
...
discordClient.on("interactionCreate", async (interaction) => {
  if (interaction.isCommand()) {
    ...
    if (commandName === "ping") {
      ...
      try {
        var serverStat = await (
          await fetch("https://iot-socket.chinhphucvn.com/stat")
        ).json();
        serverStatus = `IOT server status: \n- Online: ${serverStat.online} players.`;
        serverStatus += ` \n- Free memory: ${Math.floor(
          serverStat.freemem / 1024 / 1024
        )}/${Math.floor(serverStat.totalmem / 1024 / 1024)}MB`;
        var uptime = moment.utc(serverStat.uptime * 1000);
        var days = uptime.diff(moment.utc(0), "days");
        serverStatus += `\n- Uptime: ${
          days > 0 ? days + " days " : ""
        }${uptime.format("HH:mm:ss")}s`;
      } catch (e) {
        console.log(e);
      }
      ...
      if (!uid) return interaction.editReply("User not found!");
      ...
      var mess = new MessageEmbed()
        ...
        .setDescription(
          `${interaction.user} gửi yêu cầu hỗ trợ, vui lòng đợi quản trị viên trả lời.`
        )
        .setTimestamp();
      mess.addField("Tham gia", `<@${interaction.user.id}> @here`);
      mess.addField(
        "Hướng dẫn",
        "Bạn hãy giải thích vấn đề gặp phải và chờ quản trị viên giải quyết nhé.\nSau khi kết thúc, gõ /done để xóa kênh."
      );
      ...
      if (
        user.roles.cache.find((r) => r.name === "admin") ||
        user.roles.cache.find((r) => r.name === "moderator") ||
        user.roles.cache.find((r) => r.name === "verified-player")
      ) {
        uid = await getIOTUidFromDiscordId(userId);
        if (!uid) return channel.send("User not found!");
        else {
      ...
      await interaction.deferReply();
      var member = interaction.guild.members.cache.find(
        (r) => r.id === interaction.user.id
      );
      uid = await getIOTUidFromDiscordId(member.id);
      if (!uid) interaction.editReply("Tài khoản chưa liên kết với IOT!");
      else {
        linkIOTAccount(member, false);
        interaction.editReply("Cập nhật thành công!");
      }
    } else if (commandName === "apply-reviewer") {
      await interaction.deferReply();
      var member = interaction.guild.members.cache.find(
        (r) => r.id === interaction.user.id
      );
      var uid = await getIOTUidFromDiscordId(member.id);
      if (!uid) interaction.editReply("Tài khoản chưa liên kết với IOT!");
      else {
        var result = await functions.accountApplyReviewer(uid);
        interaction.editReply(result.message);
      }
    } else if (commandName === "resign-reviewer") {
      await interaction.deferReply();
      var member = interaction.guild.members.cache.find(
        (r) => r.id === interaction.user.id
      );
      ...
      if (!uid) interaction.editReply("Tài khoản chưa liên kết với IOT!");
        ...
        var result = await functions.accountResignReviewer(uid);
        interaction.editReply(result.message);
      }
    }
    ...
    switch (action) {
      case "approve":
        await interaction.deferReply();
        iotUid = await getIOTUidFromDiscordId(interaction.user.id);
        await functions
          .accountApproveAccount(params[1], iotUid)
          .then(async function (result) {
            await interaction.editReply(`\`\`\`${result.message}\`\`\``);
            await interaction.message.delete();
          });
        setTimeout(async function () {
          await discordRemoveChannel("review-" + params[1], "GUILD_TEXT");
        }, 10000);
        break;
      case "reject":
        await interaction.deferReply();
        iotUid = await getIOTUidFromDiscordId(interaction.user.id);
        await functions
          .accountRejectAccount(params[1], iotUid)
          .then(async function (result) {
            await interaction.editReply(`\`\`\`${result.message}\`\`\``);
            await interaction.message.delete();
          });
      ...
      case "reportignore":
        ...
        var voted = [];
        if (params[2]) voted = params[2].split(",");
        var voted_users = [];
        var voted_name = "";
        ...
        if (
          ...
          for (var i = 0; i < row.components.length; i++) {
            if (row.components[i].customId === interaction.customId) {
              ...
              else row.components[i].customId += `,${voted.length}`;
              row.components[i].label = row.components[i].label
                .split("(")[0]
                .trim();
      ...
      case "reportblock":
        ...
        voted = [];
        if (params[3]) voted = params[3].split(",");
        voted_users = [];
        for (i = 0; i < embed.fields.length; i++) {
          if (embed.fields[i].name === "Biểu quyết") {
            voted_users = embed.fields[i].value.trim().split(" ");
...
async function clearMessageInChannel(channelName) {
  var channel = discordClient.channels.cache.find(
    (c) => c.name.toLowerCase().trim() == channelName
  );
...
async function sendAllWaitingAccount() {
  var channel = discordClient.channels.cache.find(
    (c) => c.name.toLowerCase().trim() == "iot-updates"
  );
  channel.send("```Check Again!!!```");
  var accounts = (await functions.accountGetAccountReview()).data;
  if (accounts.length > 0) {
    await channel.send(
      `\`\`\`There are ${accounts.length} remaining accounts to be reviewed!\`\`\``
    );
  ...
  refReviseRoom.on("child_added", function (snap) {
    discordCreateChannel(
      ...
      if (process.env.PUBLIC == "true")
        sendChatMessage(
          `/revise/chat/${snap.key}`,
          `Tham gia Discord: ${invite} Kênh thoại Phòng ${snap.key}!`
        );
    });
  });
...
app.post("/discord_webhook", async (req, res) => {
  var data = req.body;
  var channel = discordClient.channels.cache.find(
    (c) => c.name.toLowerCase().trim() == "updates"
  );
  var msg = new MessageEmbed()
    .setTitle(
      upcaseFirst(data.action) +
        (data.resource ? " - " : "") +
        (upcaseFirst(data.resource) || "")
    )
    .setTimestamp(data.updated_at);
  if (data.actor) msg.setAuthor(data.actor.email);
  if (data) {
    var temp = data.data;
    if (temp.app) msg.addField("App", temp.app.name);
    if (temp.status) msg.addField("Status", temp.status);
    if (temp.message) msg.addField("Message", temp.message);
    if (temp.source_blob)
      msg.addField("Code Version", temp.source_blob.version);
    if (temp.release) msg.addField("Version", temp.release.version.toString());
    if (temp.state) msg.addField("State", temp.state);
  }
  await channel.send({
    embeds: [msg],
  });
...
app.post("/iot_chat", (req, res) => {
  ...
  var data = req.body;
  var channel = discordClient.channels.cache.find(
    (c) => c.name.toLowerCase().trim() == "iot-chat"
  );
