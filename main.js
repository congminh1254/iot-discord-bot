		...
		category = guild.channels.cache.find(
			(c) =>
				c.name.toLowerCase().trim() ==
					category_name.toLowerCase().trim() &&
...
async function discordLockAccount(interaction, uid) {
	try {
		...
		switch (reaction.emoji.name) {
			case "✅":
				var days_msg = await confirm_msg.channel.send(
					"```How many days?```"
				);
		...
		case "/acc":
			...
			if (Object.values(data).length == 0) {
				...
				var mess = new MessageEmbed()
					...
					.addFields(
						...
						},
						{
							name: "Ranking",
							value:
								utils.getRankGradeName(user.talent) || "null",
							inline: true,
						},
						{
							name: "Birthday",
				...
				if (user.ip) {
					...
					mess.addField(
						"IP",
						`${ip} - ${joIPData.country} - ${joIPData.as}`
					);
				}
...
async function linkIOTAccount(member, welcome_message = true) {
	...
	if (users.length > 0) {
		...
		if (welcome_message)
			channel.send(
				`Chào mừng người chơi IOT ${
					user.username
				} (${utils.getRankGradeName(
					user.talent
...
function generateIOTProfile(uid) {
	return new Promise(async function (resolve, reject) {
		...
		var created_time =
			public_user.created_at || Math.floor(new Date() / 1000);
		...
		switch (action) {
			...
			case "reportignore":
				...
				if (
					...
					for (var i = 0; i < row.components.length; i++) {
						if (
							row.components[i].customId === interaction.customId
						) {
							if (params.length <= 2)
								row.components[
									i
								].customId += `_${voted.length}`;
							else
								row.components[
									i
								].customId += `,${voted.length}`;
							row.components[i].label = row.components[i].label
								.split("(")[0]
								.trim();
							voted_name = row.components[i].label;
							row.components[i].label += ` (${
								voted.length + 1
							} phiếu)`;
							voted = row.components[i].customId
								.split("_")[2]
								.split(",");
						}
					}
			...
			case "reportblock":
				...
				if (
					...
					var result = await functions.accountLockAccount(
						uid,
						minutes,
						reason
					);
